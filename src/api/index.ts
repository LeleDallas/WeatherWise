...
export default {
    cities: {
        fetch: async (name: string, lng: string) => await api.get(`search?name=${name}&count=10&language=${lng}&format=json`).then((response) => response.data),
    },
    weather: {
        fetchForecast: async (latitude: number, longitude: number) =>
            await weatherApi
                .get(
                ...
                .then((response) => {
                    ...
                    return forecasting.daily.time.reduce(
                        (acc, date, index) => {
                            acc[date] = {
                                precipitation_probability_max: {
                                    ...
                                    title: 'precipitation_probability_max',
                                    icon: 'icon-Rain3',
                                },
                                wind_speed_10m_max: {
